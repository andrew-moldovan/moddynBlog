module moddynBlog {
  'use strict';
  
  /** @ngInject */
  export function lineCanada(): ng.IDirective {

    return {
      restrict: 'E',
      scope: {
        data: '='
      },
      templateUrl: 'app/components/posts/milk/canada.html',
      controller: MilkCanadaController,
      controllerAs: 'mcc',
      link: (scope: any, ele: JQuery, attrs: ng.IAttributes) => {
        scope.mcc.d3Service.d3().then(d3 => {
          scope.mcc.d3HelperService.getData('/assets/data/milk/Canada_All.json').then((data: any) => {
            
            var margin = { top: 20, right: 80, bottom: 30, left: 50 },
              width = 960 - margin.left - margin.right,
              height = 500 - margin.top - margin.bottom;

            var parseDate = d3.time.format("%Y/%m").parse;

            var x = d3.time.scale()
              .range([0, width]);

            var y = d3.scale.linear()
              .range([height, 0]);

            var color = d3.scale.category10();

            var xAxis = d3.svg.axis()
              .scale(x)
              .orient("bottom");

            var yAxis = d3.svg.axis()
              .scale(y)
              .orient("left");

            var line = d3.svg.line()
              .interpolate("basis")
              .x(function(d) { return x(d.Ref_Date); })
              .y(function(d) { return y(d.litres); });

            var svg = scope.mcc.d3HelperService.createSVG(d3, width, height, margin, scope.mcc.data, ele, 'line-canada');

            color.domain(d3.keys(data[0]).filter(function(key) { 
              return key !== "Ref_Date"; 
            }));

            data.forEach(function(d) {
              d.Ref_Date = parseDate(d.Ref_Date);
            });

            var cities = color.domain().map(function(name) {
              return {
                name: name,
                values: data.map(function(d) {
                  var litres = d[name];
                  if (!litres || litres !== litres) {
                    litres = 0;
                  }
                  return { date: d.Ref_Date, litres: litres };
                })
              };
            });

            x.domain(d3.extent(data, function(d) { return d.Ref_Date; }));

            y.domain([
              d3.min(cities, function(c) { 
                return d3.min(c.values, function(v) { 
                  return v.litres; 
                }); 
              }),
              d3.max(cities, function(c) { 
                return d3.max(c.values, function(v) { 
                  return v.litres; 
                }); 
              })
            ]);

            svg.append("g")
              .attr("class", "x axis")
              .attr("transform", "translate(0," + height + ")")
              .call(xAxis);

            svg.append("g")
              .attr("class", "y axis")
              .call(yAxis)
              .append("text")
              .attr("transform", "rotate(-90)")
              .attr("y", 6)
              .attr("dy", ".71em")
              .style("text-anchor", "end")
              .text("Litres drunk per capita (L)");

            var city = svg.selectAll(".city")
              .data(cities)
              .enter().append("g")
              .attr("class", "city");

            city.append("path")
              .attr("class", "line")
              .attr("d", function(d) { return line(d.values); })
              .style("stroke", function(d) { return color(d.name); });

            city.append("text")
              .datum(function(d) { return { name: d.name, value: d.values[d.values.length - 1] }; })
              .attr("transform", function(d) { return "translate(" + x(d.value.date) + "," + y(d.value.litres) + ")"; })
              .attr("x", 3)
              .attr("dy", ".35em")
              .text(function(d) { return d.name; });
            

          });
        });
      },
      bindToController: true
    };
  }

  /** @ngInject */
  class MilkCanadaController {

    constructor(private d3Service: any, private d3MultiLineChartHelperService: any, private d3HelperService: any) {
    }
  }
}
